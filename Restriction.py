...
# Restriction enzymes
commercial_enzymes = {str(x): getattr(Restriction, str(x)) for x in sorted(list(Restriction.CommOnly))}
...
def re_digest_analysis(sequence_to_cut: dict, sequence_to_not_cut: dict = {}, enzymes: dict = commercial_enzymes, min_cuts: int = 0, max_cuts: int = 9999999999, output_style: str = "map"):
  ...
  if output_style not in ["list", "number", "map"]:
    raise ValueError("output_style must be either 'list', 'number' or 'map'")
  if min_cuts < 0:
  ...
  if sequence_to_not_cut:
    ...
    for i,seq in enumerate(sequence_to_not_cut.keys()):
      re_analysis_not_cut = Restriction.Analysis(re_enzymes, sequence_to_not_cut[seq], linear=True)
    # Exclude enzymes that cut the sequence from the enzyme batch
      enzymes_that_cut_restricted_seq.extend([k for k, v in re_analysis_not_cut.full().items() if v != []])
  ...
  if min_cuts != 0 or max_cuts != 9999999999:
    ...
    for i,seq in enumerate(sequence_to_cut.keys()):
      ...
      re_analysis_cut = Restriction.Analysis(re_enzymes, sequence_to_cut[seq], linear=True)
      
      enzymes_that_cut_less_than_min_cuts.extend([k for k, v in re_analysis_cut.full().items() if len(v) < min_cuts and k not in enzymes_that_cut_less_than_min_cuts])
      
      enzymes_that_cut_more_than_max_cuts.extend([k for k, v in re_analysis_cut.full().items() if len(v) > max_cuts and k not in enzymes_that_cut_more_than_max_cuts])
      
      
  # Exclude enzymes that fail filter from the enzyme batch
  enzymes_to_remove = list(set(enzymes_that_cut_restricted_seq + enzymes_that_cut_less_than_min_cuts + enzymes_that_cut_more_than_max_cuts))
  if len(enzymes_to_remove) > 0:
    re_enzymes = Restriction.RestrictionBatch([k for k, v in re_analysis_cut.full().items() if k not in enzymes_to_remove])
  ...
  with io.StringIO() as buf, redirect_stdout(buf):
    for i,seq in enumerate(sequence_to_cut.keys()):
      ...
      re_analysis = Restriction.Analysis(re_enzymes, sequence_to_cut[seq], linear=True)
  ...
  # TODO TEST IF THESE CUSTOM OUTPUT FUNCTIONS WORK ON ANY OTHER OUTPUT SETTING OTHER THAN 'map'
      ...
      if sequence_to_not_cut:
        print(re_analysis._make_nocut_only(nc = enzymes_that_cut_restricted_seq, s1 = f"{tab}Enzymes that cut restricted sequences{nl}{nl}"))
      
      if min_cuts != 0: 
        print(re_analysis._make_nocut_only(nc = enzymes_that_cut_less_than_min_cuts, s1 = f"{tab}Enzymes that cut less than {min_cuts} times{nl}{nl}"))
      
      if max_cuts != 9999999999:
        print(re_analysis._make_nocut_only(nc = enzymes_that_cut_more_than_max_cuts, s1 = f"{tab}Enzymes that cut more than {max_cuts} times{nl}{nl}"))
...
test = re_digest_analysis(sequence_to_cut = test_seq, sequence_to_not_cut = test_restricted_seq, enzymes = commercial_enzymes, min_cuts=0, max_cuts=1, output_style = "map")
print(test)